"""Importando abc"""
from abc import ABC, abstractmethod

class FiguraGeometrica(ABC):
    """Clase abstracta que define la interfaz para calcular áreas."""

    @abstractmethod
    def calcular_area(self):
        """Método abstracto para calcular el área de una figura."""
        return

    @abstractmethod
    def setcalcular_area(self, a, b):
        """Método abstracto para settear datos."""
        return

#  Clase del rectangulo
class Rectangulo(FiguraGeometrica):
    """Inicializacion de rectangulo"""
    def __init__(self, base, altura):
        self.base = base
        self.altura = altura

    def calcular_area(self):
        """Calcula el área del rectángulo."""
        if self.base <= 0:
            return print("El base es menor igual a 0")

        if self.altura <= 0:
            return print("El altura es menor igual a 0")
        area = self.base*self.altura
        return area

    def setcalcular_area(self, a, b):
        self.base = a
        self.altura = b

#Clase del triangulo
class Triangulo(FiguraGeometrica):

    """Inicializacion de triangulo"""
    def __init__(self, base, altura):
        self.base = base
        self.altura = altura

    def calcular_area(self):
        """Calcula el área del triángulo."""
        if self.base <= 0:
            return print("El base es menor igual a 0")

        if self.altura <= 0:
            return print("El altura es menor igual a 0")
        area = (self.base*self.altura)/2
        return area

    def setcalcular_area(self, a, b):
        self.base = a
        self.altura = b

#Clase del circulo
class Circulo(FiguraGeometrica):
    """Inicializacion de circulo"""
    def __init__(self, radio):
        self.pi = 3.14159
        self.radio = radio

    def calcular_area(self):
        """Calcula el área del círculo."""
        if self.radio <= 0:
            return print("El radio es menor igual a 0")

        area = self.pi*(self.radio**2)
        return area

    def setcalcular_area(self, a, b):
        self.radio = a

# Variables globales
BASE_RECTANGULO = 10
ALTURA_RECTANGULO = 5
BASE_TRIANGULO = 7
ALTURA_TRIANGULO = 4
RADIO_CIRCULO = 3

# Ejecución principal
if __name__ == "__main__":
    # Crear instancias de las figuras
    rectangulo = Rectangulo(BASE_RECTANGULO, ALTURA_RECTANGULO)
    triangulo = Triangulo(BASE_TRIANGULO, ALTURA_TRIANGULO)
    circulo = Circulo(RADIO_CIRCULO)

    # Calcular áreas
    print(f"El área del rectángulo es: {rectangulo.calcular_area()}")
    print(f"El área del triángulo es: {triangulo.calcular_area()}")
    print(f"El área del círculo es: {circulo.calcular_area()}")
